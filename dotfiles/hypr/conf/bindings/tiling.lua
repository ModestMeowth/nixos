...
for w = 1, 6 do
  ...
  hl.bind(
    "SUPER+SHIFT+" .. key,
    hl.dsp.window.move({ workspace = tostring(w) }, { description = "Move Window to Workspace" .. w })
  )
  hl.bind(
    "SUPER+SHIFT+ALT+" .. key,
    hl.dsp.window.move(
      { workspace = tostring(w), follow = false },
      { description = "Silently move Window to Workspace" .. w }
    )
  )
end

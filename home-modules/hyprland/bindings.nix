...
in
{
  wayland.windowManager.hyprland.settings = {
    bind = [
      (bind "${mainMod}+RETURN" (exec "xdg-terminal-exec") { description = "Open Terminal"; })

      (bind "${mainMod}+E" (exec "launch-file-manager") { description = "Open File Manager"; })

      (bind "${mainMod}+D" (exec ''launch-browser --profile=\"Default\"'') { description = "Open Browser"; })

      (bind "${mainMod}+J" (exec ''launch-browser --profile=\"Work\"'') { description = "Open Browser (Work profile)"; })
      ...
      (bind "${mainMod}+SHIFT+Q" (exec "hyprshutdown") { description = "Exit Hyprland"; })
